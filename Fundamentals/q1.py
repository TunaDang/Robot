...
def calculate_distances(x, y):
    ...
    diff = x - y
    squared = diff ** 2
    summed = np.sum(squared, axis = 1).reshape(3,1)
...
def combine_squares(square_1, square_2, square_3, square_4):
    ...
    combined_square = torch.cat((top, bottom), axis = 0)
...
def video_to_filmstrip(video_frames):
    ...
    """
    torch_frames = [torch.from_numpy(frame) for frame in video_frames]
    return torch.cat(torch_frames, dim = 1)